...
class TransactionRoomReservationController extends Controller
{
    ...
    public function viewCountPerson(User $customer)
    {
        return view('transaction.reservation.viewCountPerson', compact('customer'));
    }

    public function chooseRoom(ChooseRoomRequest $request, User $customer)
    {
    ...
    public function confirmation(User $customer, Room $room, $stayFrom, $stayUntil)
    {
        $price = $room->price;
    ...
    public function payDownPayment(User $customer, Room $room, Request $request, TransactionRepository $transactionRepository, PaymentRepository $paymentRepository)
    {
        $occupiedRoomId = $this->getOccupiedRoomID($request->check_in, $request->check_out);
